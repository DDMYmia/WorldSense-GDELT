...
**Amazon OpenSearch Service**: Acts as the core analytical engine optimized for cost and performance. The cluster configuration underwent aggressive optimization, reducing from the initial expensive configuration to cost-effective instances, achieving significant cost reduction. The current setup includes 2 data nodes and 2 master nodes with 10GB GP3 storage each, eliminating Multi-AZ with Standby and Auto-Tune features to maximize cost savings. Pre-defined index mappings ensure optimal query performance for time-series, geospatial, and sentiment analysis operations.
